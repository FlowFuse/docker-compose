...
async function run(options) {
    ...
    for (let i = 0; i < options.packages.length; i++) {
        if (!existsSync(path.join(options.packageDir, options.packages[i]))) {
            try {
                ...
                log(`${chalk.greenBright('+')} ${options.packages[i]}`)
            } catch(err) {
    ...
    try {
        await runner(
            'forge-ui-components',
            `${NPM} run build`,
            { cwd: path.join(options.packageDir, 'forge-ui-components') }
        )
        log(`${chalk.greenBright('+')} flowforge/forge-ui-components`)
    } catch(err) {
        log(`${chalk.greenBright('-')} flowforge/forge-ui-components`)
        error('Failed to build forge-ui-components')
        error(err.stderr)
        error('Aborting')
        return
    }
